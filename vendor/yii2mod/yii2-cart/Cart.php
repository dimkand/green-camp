...
class Cart extends Component
{
    ...
    protected function addItem(CartItemInterface $item)
    {
        ...
        if(isset($this->items[$uniqueId])){
            $quantity = $this->items[$uniqueId]->getQuantity();
            $this->items[$uniqueId]->setQuantity(++$quantity);
        }
        else
    ...
    public function getAttributeTotal($attribute, $itemType = null): int
    {
        ...
        foreach ($this->getItems($itemType) as $model) {
            if($attribute != 'quantity')
                $sum += $model->{$attribute} * $model->getQuantity();
            else
                $sum += $model->{$attribute};
        }
    ...
    public function getAttributeTotalById($attribute, $id, $itemType = null): int
    {
        ...
        if($attribute == 'quantity')
            return (int)$items[$id]->{$attribute};

        return (int)$items[$id]->{$attribute} * $items[$id]->getQuantity();
    }
